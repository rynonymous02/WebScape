...
import { 
  ChevronRight, ChevronDown, Frame, Square, Circle, PenTool, Type, 
...
export const LayerPanel: React.FC = () => {
  ...
  const renderLayerItem = (nodeId: string, depth: number = 0) => {
    ...
    const handleDragOver = (e: React.DragEvent) => {
      ...
      if (node.type === 'frame' && relY > 6 && relY < rect.height - 6) {
        setDropPos('inside');
    ...
    const handleDrop = (e: React.DragEvent) => {
      ...
      if (dropPos === 'inside' && node.type === 'frame') {
        ...
        const siblings = parentId ? project.nodes[parentId]?.children || [] : project.rootNodeIds;
        const targetIdx = siblings.indexOf(node.id);
        reorderNode(draggedNodeId, parentId, targetIdx + 1);
      }
    ...
    return (
      <div key={node.id} className="flex flex-col relative select-none">
        ...
        {isDropTarget && dropPos === 'above' && (
          <div className="h-0.5 bg-indigo-500 rounded-full my-0.5 mx-2 shadow-[0_0_8px_rgba(99,102,241,1)]" />
        )}
        ...
        <div
          ...
          className={`h-7 flex items-center justify-between px-2 text-xs rounded transition-all cursor-pointer group ${
            isSelected
          ...
          } ${isDropTarget && dropPos === 'inside' ? 'ring-1 ring-indigo-500 bg-indigo-500/20' : ''}`}
          style={{ paddingLeft: `${Math.max(8, depth * 14 + 8)}px` }}
        >
          {/* Left: Drag Handle, Expand arrow, Icon, Name */}
          <div className="flex items-center gap-1.5 truncate flex-1">
            <GripVertical className="w-3 h-3 opacity-0 group-hover:opacity-60 text-slate-400 shrink-0 cursor-grab" />
            ...
            {editingId === node.id ? (
              <input
                ...
                autoFocus
                className={`text-xs px-1.5 py-0.5 rounded outline-none border border-indigo-500 w-28 ${
  ...
  return (
    <aside className={`w-60 border-r flex flex-col h-full select-none z-20 transition-colors ${
      isDark ? 'bg-slate-900 border-slate-800 text-slate-300' : 'bg-white border-slate-200 text-slate-800'
    }`}>
      {/* Panel Header */}
      <div className={`h-10 border-b px-3 flex items-center justify-between font-semibold ${
